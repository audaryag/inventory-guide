...
from spec import CANVAS, PAGES, CARDS, SLICERS, VISUALS
...
def steps():
    ...
    S.append(dict(
        title="Create the five pages",
        page="—",
        do=["At the bottom of the window, click the + button five times.",
    ...
    S.append(dict(
        title="Copy the header band to every page",
        page=PAGES[0],
        do=["Click the first card, then Ctrl+click the other four cards and both slicers "
            "(seven things selected).",
            "Ctrl+C.",
            "Go to each of the other four pages and press Ctrl+V."],
        fields=[], note="Positions travel with the copy, so the band lands in exactly the "
    ...
    S.append(dict(
        ...
        do=["Ribbon: View → tick Sync slicers. A pane opens on the right.",
            "Click the Month slicer, then tick both Sync and Visible for all five pages.",
            "Do the same for the Plant slicer."],
...
def part4_markdown():
    L = ["# PART 4 — Build the pages", "",
         ...
         "**Create the five pages** with the **+** at the bottom, named: " +
    ...
    L += ["", "**4.2** Two **Slicer** visuals (**Insert → Slicer**), each set to",
    ...
    L += ["", "**4.3** Select all seven → **Ctrl+C** → on each other page **Ctrl+V**. "
          "Positions come with them.", "",
          "**4.4** Ribbon **View** → tick **Sync slicers**; for each slicer tick **Sync** and",
          "**Visible** on all five pages. Without it, two pages can disagree about the same "
          "month.", ""]
