...
VISUALS = [
    ...
    ("Summary", "Matrix",
     'Inventory Summary — Inventory (TB), Inventory (MB5B) and Difference',
     [("Rows", ["dimPlant[Plant]", "dimCategory[Category]"]),
      ("Columns", ["dimDate[MonthName]"]),
      ("Values", ["TB Inventory Rs Cr" + AS + "Inventory (TB)",
                  "Inventory Rs Cr" + AS + "Inventory (MB5B)",
                  "Difference Inventory Rs Cr" + AS + "Difference"]),
      ("Filters", ["In Summary Window  →  is 1"])],
     (16, 88, 1248, 300),
     'One table, the shape the Excel sheet has it: a row per plant that opens into RM, FG and Consumables, and across the top the three master columns — what the books say, what the stock report says, and the difference — with the months underneath each. Read straight across a row and you are comparing the books with the stock for one plant, one type, one month.',
     ['Rows takes two fields, in this order: dimPlant[Plant], then dimCategory[Category]. The table opens on the plant and the + beside it opens RM, FG and Consumables underneath.', 'Columns takes one field: dimDate[MonthName].', 'Values takes three measures, in this order: TB Inventory Rs Cr, Inventory Rs Cr, Difference Inventory Rs Cr — renamed Inventory (TB), Inventory (MB5B) and Difference by double-clicking each one in the Values box. One table, every figure on the row: the books, the stock report and the difference under each month. A Metric field above the month would read closer to the Excel sheet, but Desktop opens a two-level column hierarchy collapsed and sometimes draws it as an empty card, so the three measures go in as values instead.', 'Filters pane → drag the measure In Summary Window in → is 1. That gives the newest March plus the three most recent months by default, and your ticks instead when you tick months in the slicer.', 'Format pane → Subtotals → Column subtotals: Off. Stock is a level, not a flow: a Total column would add March to July, the same steel counted twice.', 'Format pane → Subtotals → Row subtotals: On. The Grand Total row adds the plants and types inside one month, which is a real figure: one point in time.', 'Format pane → Row headers → Values → Font size: 8; Column headers → Font size: 8, Word wrap: On; Values → Font: Arial, Font size: 8, Colour: #1F2A24.']),
    ...
    ("FG", "Matrix",
     ...
     (16, 88, 1248, 148),
     'Finished goods per plant, in one table the way the Excel sheet has it: IN MW, IN CRS and IN DAYS across the top, the months underneath each, a row per plant and a Grand Total row.',
     ['Rows takes one field: dimPlant[Plant].', 'Columns takes one field: dimDate[MonthName].', 'Values takes three measures, in this order: Inventory MW, Inventory Rs Cr, Days — renamed MW, Rs Cr. and Days by double-clicking each one in the Values box. That is what makes it one table rather than three, with MW, crores and days under each month. A Measure field above the month would put the blocks in the Excel sheet’s order, but Desktop opens a two-level column hierarchy collapsed and sometimes draws it as an empty card, so the measures go in as values instead.', 'Filters pane → dimCategory[Category] → tick FG only, then the measure In Summary Window → is 1.', 'Format pane → Subtotals → Column subtotals: Off. Stock is a level, not a flow: a Total column would add March’s stock to July’s, the same panels counted twice. Row subtotals: On — that adds the plants inside one month, which is the Grand Total row the Excel sheet had.', 'Format pane → Values → Font: Arial, Font size: 8, Colour: #1F2A24; Row headers → Font size: 8; Column headers → Font size: 8.', 'Days is MW ÷ capacity MW, so a plant with no row on the MW Capacity sheet is blank there on purpose — a missing denominator is not the same as no stock.']),

    ("FG", "Matrix",
     'Inventory FG Details — By Techno',
     [("Rows", ["dimNature[Nature]"]),
      ...
      ("Values", ["Inventory MW" + AS + "MW",
                  ...
                  "Days" + AS + "Days"]),
      ("Filters", ["dimCategory[Category]  →  is FG",
                   "dimPlant[Plant]  →  Jaipur Module and Dholera Module",
                   "dimNature[Nature]  →  is not Unassigned",
                   "In Summary Window  →  is 1"])],
     (16, 244, 1248, 164),
     'The same three blocks by module technology rather than by plant — G12 Perc, G12R Topcon, M10 Perc, M10 Topcon and the rest — which is where a build-up in one technology shows itself. Modules only, as the Excel sheet has it.',
     ['Rows takes one field: dimNature[Nature] — the technology.', 'Columns takes one field: dimDate[MonthName], and Values the same three measures as the block above it, in the same order.', 'Filters pane → dimCategory[Category] → tick FG only; the measure In Summary Window → is 1; then dimPlant[Plant] → tick Jaipur Module and Dholera Module only, and dimNature[Nature] → untick Unassigned. Those last two are why this table holds the module technologies and nothing else, which is how the Excel sheet has it: a cell is not a module technology, and a material the FG Master does not carry is not one either. The plant table above still counts every plant and every material, so nothing is hidden from the totals.', 'Format pane → Subtotals → Column subtotals: Off, Row subtotals: On.', 'Format pane → Values → Font: Arial, Font size: 8, Colour: #1F2A24; Row headers → Font size: 8; Column headers → Font size: 8.']),
    ...
    ("RM", "Matrix",
     'RM Inventory Plant Wise — Rs Cr and Days',
     [("Rows", ["dimPlant[Plant]"]),
      ("Columns", ["dimDate[MonthName]"]),
      ...
      ("Filters", ["dimCategory[Category]  →  is RM",
                   "In Summary Window  →  is 1"])],
     (16, 88, 1248, 148),
     'Raw material and packing per plant, in one table: crore rupees and days of cover under each month, a row per plant and a Grand Total row — the top block of the old RM sheet.',
     ['Rows takes one field: dimPlant[Plant]. Columns takes one field: dimDate[MonthName].', 'Values takes two measures, in this order: Inventory Rs Cr and Days, renamed Rs Cr. and Days by double-clicking each one in the Values box — one table with both figures under each month, the way the old RM sheet had IN CRS and IN DAYS side by side. MW is left out because an RM megawatt figure is derived from a BOM rather than measured.', 'Filters pane → dimCategory[Category] → tick RM only, then In Summary Window → is 1.', 'Format pane → Subtotals → Column subtotals: Off. Stock is a level, not a flow: a Total column would add March’s steel to July’s, the same steel counted twice. Row subtotals: On — that adds the plants inside one month, which is the Grand Total row the Excel sheet had.', 'Format pane → Values → Font: Arial, Font size: 9, Colour: #1F2A24.', 'Clicking a plant row filters the nature block and both charts below it.']),

    ("RM", "Matrix",
     'RM Inventory by Group Nature and Nature — Rs Cr and Days',
     [("Rows", ["factInventory[GroupNature]", "dimNature[Nature]"]),
      ...
      ("Filters", ["dimCategory[Category]  →  is RM",
                   "In Summary Window  →  is 1"])],
     (16, 244, 1248, 232),
     'The second block of the old sheet, both figures together: Module and Cell, each opening into its natures — cell cost, frame, glass, POE, wafer, paste, screens, gases and the rest — with a subtotal on each group and a grand total under them.',
     ['Rows takes two fields, in this order: factInventory[GroupNature], then dimNature[Nature]. Columns takes dimDate[MonthName], and Values the same two measures as the block above, in the same order.', 'Format pane → Row headers → Stepped layout: Off, +/- icons: On, so Group Nature and Nature get a column each with an expander on each group.', "Format pane → Subtotals → Row subtotals: On with 'Per row level' On, so Total Module and Total Cell both appear and not only the grand total.", 'Format pane → Subtotals → Column subtotals: Off.', "A nature reading Unassigned is a material the RM master does not carry — it is money the report will not silently file under someone else's nature, so it is shown rather than dropped."]),
